...
**`go test ./...` is green right now with all 25 defects live.** A green suite is not an
acceptance signal. A task is done when a test exists that **fails before your change and passes
after it**. If your new test passes before you touch the implementation, it is testing the wrong
thing — rewrite it.

## Verified state of the tree (2026-08-03, commit `7069923`)
...
- `internal/gen`, `cmd/talaria/run.go` and `internal/corpus/lock_other.go` **do not exist**.
  Findings 7, 12 and 26 are moot. Finding 4's `run` half, finding 19's `--report junit` half and
  finding 16's `run` pathological case are all gone with them.
- `grep -rn "credentials_withheld\|allow-host\|allow_hosts"` over `*.go` returns **zero hits**.
  The whole of §5a "Credentials bind to hosts" is unimplemented.
- Several line numbers in the findings report have drifted. Corrections carried into the tasks
  ...
  `entry.go:119` (not 121). **Grep for the symbol, do not trust a line number.**
...
amendment. It is not scoped as work here; Task 4's doc pass should not reintroduce the conflict.
...
- `internal/request/build.go` (modify) — `firstServer` substitutes; add an exported
...
1. A spec with `servers[0].url: "https://{region}.api.example.com/v1"` and
   ...
   http(s) URL"* — reproduce that failure first.
...
1. A variable `default` of `evil.com/` or `evil.com#` — substitution must not let a variable value
   ...
   same class as the `url.PathEscape` rule already enforced in `binder.path`.
...
1. In `internal/request/build.go`, add `substituteServer(url string, vars map[string]*v3high.ServerVariable) (string, error)` doing one left-to-right pass over `{name}` spans.
...
3. `firstServer` returns the substituted `Servers[0].URL`; add `ServerURLs` for Task 2.
4. Report failures through `b.fail` so they join the existing collected-problems error.
...
- `cmd/talaria/flags_test.go` (modify) — `--allow-host` is registered and repeatable

**Implementation files:**
- `internal/config/config.go` (modify) — add `AllowHosts []string \`yaml:"allow_hosts"\`` to `Profile`
- `internal/request/hosts.go` (create) — `HostSet` type, `NewHostSet(specURLs, allowFlags, profileHosts []string) HostSet`, `(HostSet) Allows(rawURL string) bool`, `(HostSet) Key(rawURL string) string`
- `cmd/talaria/root.go` (modify) — register `--allow-host` as a repeatable persistent flag

**Red — write failing tests:**
1. A profile file containing `allow_hosts: ["localhost:9000"]` loads without error. **Today this
   is a parse error** because `Config` decodes with `KnownFields(true)` — write that failing test
   first, it is the regression the finding names.
...
2. Write `internal/request/hosts.go`: `HostSet` wraps a `map[string]bool` of `host:port` keys plus
   ...
   empty hosts, and checks both maps.
3. Register `root.PersistentFlags().StringArray("allow-host", nil, …)` beside `--base-url` in
   `newRootCmd`, so `call`, `auth check` and `history replay` all accept it.
...
**Implementation files:**
- `internal/request/request.go` (modify) — add `Withheld []Withheld` to `Request`; `Withheld{Scheme, Reason, Host string}`
- `internal/request/build.go` (modify) — `Inputs.Hosts HostSet`; `binder.credentials` diverts
- `cmd/talaria/call.go` (modify) — render `credentials_withheld`, stderr warning
- `cmd/talaria/history.go` (modify) — **rewrite the replay path**; delete `replayRequest`,
  `replayQuery`, `replayPairs`, `replayValue`, `replayableEnv`, `encodingPrefix`
...
1. Spec server `https://api.example.com`, `TALARIA_AUTH_BEARER` set to a canary, `--base-url`
   ...
   passes today.
2. The same call emits `credentials_withheld: [{"scheme":"bearerAuth","reason":"…","host":"127.0.0.1:PORT"}]` in the JSON envelope, and exactly one line on stderr naming the scheme and the host.
...
6. `auth check` against an off-set `--base-url` reports the scheme as not-going-to-be-sent, so
   "present" never means "will actually be sent" (§5a).
...
6. A path-parameter value recovered from the stored path containing `../`, a CRLF, or a `%00` —
   ...
   `SplitsRequest` gates actually catch it on this path too.
...
*Host binding:*
1. Add `Withheld` to `internal/request`, and `Hosts HostSet` to `Inputs`.
2. In `binder.credentials`, for each credential test `b.in.Hosts.Allows(req.BaseURL)`. On failure
   append `Withheld{Scheme: cred.Scheme, Reason: "host not in spec servers[]", Host: hosts.Key(req.BaseURL)}` instead of the `Pair`.
3. In `cmd/talaria/call.go`, build the `HostSet` from `request.ServerURLs(doc)` ∪ `--allow-host` ∪
   `prof.AllowHosts`, pass it into `Inputs`, and render `req.Withheld` as `credentials_withheld`
   in `callView` plus one stderr line.
...
**Implementation files:**
- `internal/config/auth.go` (modify) — `Supported bool` on `Credential`; `Schemes` emits
  unsupported; `credentialFor` satisfies an unsupported scheme from `TALARIA_AUTH_BEARER`;
  `auth.go:167` becomes `clierr.CredentialMissing`
- `cmd/talaria/auth.go` (modify) — `authScheme.Supported`; `satisfied` stops treating
  `config.Unsupported` as a no-op
...
5. **The agreement test, which is the point:** for a matrix of specs (`oauth2`, `openIdConnect`,
   ...
   verdict and `call`'s exit code never disagree. DESIGN.md calls this non-negotiable: *"`auth
...
**Green — minimal implementation:**
1. Add `Supported bool` to `config.Credential` (which has five fields today and no way to express
   "declared but unsupported") and to `cmd/talaria`'s `authScheme` view (three fields, same gap).
...
4. `credentialFor` satisfies an unsupported requirement from `secret.Env(EnvBearer)` when set.
5. `internal/config/auth.go:167` changes from `clierr.Usage` (exit 2) to `clierr.CredentialMissing`
   (exit 5), naming the scheme and `TALARIA_AUTH_BEARER`. Note `cmd/talaria/auth.go:155,158`
   already reports the neighbouring condition with `CredentialMissing` — that inconsistency is the
   finding.
6. `satisfied` (`cmd/talaria/auth.go:173`) — **the bug is line 188, not line 182.** `case
   config.Unsupported:` is empty, so `usable` stays false, and `return !usable` then returns
   **true**: an operation whose only alternative is OAuth2 is reported satisfied and exits 0, while
   `call` on the same operation exits 2. Give `Unsupported` an explicit verdict rather than an
...
5. Ordinary media types (`application/json`, `application/vnd.api+json`, `text/plain; charset=utf-8`) still pass. **Do not break the `; charset=` form** — a naive token check will.
...
**Green — minimal implementation:**
1. In `document.body` (`config.go:284`), call `checkSplit("header", "Content-Type", req.Body.ContentType)` and return its error before writing the directive at line 290. `checkSplit`
   has exactly two call sites today — `config.go:221` (headers) and `config.go:250` (cookies) — and
   line 290 is the only `d.directive("header", …)` in the file that bypasses it.
2. In `binder.contentType` (`body.go:136`), reject a media type that is not `type/subtype` with
   ...
   (`rb.Content[0].ContentType`, line 148); the `--header` branch at 137-141 is already CRLF-checked.
3. Apply the same guard on the render path so the emitted curl is safe to print.

**Note while you are in `contentType`:** line 139 returns `h.Value.String()` — the *redacted*
...
1. Read `.ralph/refactor-backlog.md` first — build iterations record structural problems there as
   ...
   you deliberately did not do, with one line on why. **The file does not exist yet** — if it is
   still absent, note that and move on; do not create a placeholder.
2. Read every file touched since the start of the phase.
3. Consolidate types and helpers duplicated because separate iterations could not see each other's
   work. Specific suspects after tasks 1–5: host-set construction is now built in both
   `cmd/talaria/call.go` and `cmd/talaria/auth.go` and must exist once; `Withheld` rendering may
   have been written twice; media-type validation exists at two gates by design but should share
   one predicate.
4. Move logic that accumulated in `cmd/talaria` down into the package that owns it. **`cmd/talaria`
   held 28% of production code before this phase and is the largest single component** — CLAUDE.md
   forbids adding to it. Measure the line count before and after and put both in the commit message.
5. Delete commented-out code, superseded helpers, and any comment asserting a property no test
   enforces — either write the test or delete the claim.
6. **Drain the debris the `talaria run` deletion left behind.** This was surveyed and is
   confirmed present; it is the largest single source of net-negative lines available in the phase:
   - `cmd/talaria/call.go:377-389` — `validateWith` has **zero callers**. Its only reason to exist
     was `run` reusing a prebuilt `*validate.Validator`, as its own comment at 378-379 says.
   - `internal/output/output.go:47-59` — `parseFormat`'s `kind` parameter now has exactly one
     caller (`ParseFormat`, line 46, passing `"output"`). The generalisation existed for `--report`.
   - `cmd/talaria/agentdoc_test.go:164-166` — the `notRunnable` map is keyed on the literal doc line
     `talaria run ./openapi.yaml --tag pets …`. That line no longer appears in README, AGENT.md or
     DESIGN.md (verified), so the key is dead. Removing it restores the map's stated purpose:
     editing a doc line should bring its reason back for review.
   - Stale `run` rationale in comments, all describing deleted code:
     `internal/curl/config.go:149` and `:179-180`; `internal/canary/canary_test.go:473-475`;
     `internal/output/output.go:49`; `cmd/talaria/call.go:378,392`; `cmd/talaria/history.go:406`;
     `internal/corpus/entry.go:37,55`; `internal/corpus/store.go:29`; `internal/curl/version.go:27-28`;
     `internal/secret/response.go:240`; `internal/validate/validate.go:67`;
     `internal/curl/config_test.go:102`. Rewrite each to state the reason that still applies, or
     delete it. Several are load-bearing design rationale with a dead example attached — do not
     delete the reasoning along with the example.
7. Record every pattern you consolidated in `CLAUDE.md`, so later iterations follow it instead of
   re-inventing it. The host-set rule in particular belongs there beside "Credentials bind to hosts".
...
**Implementation files:**
- `internal/request/request.go` (modify) — `Body` gains its origin (`BodyArgv`, `BodyFile`, `BodyStdin` + path)
- `internal/request/body.go` (modify) — `binder.body` records which of the three sources `--body` named
- `cmd/talaria/call.go` (modify) — apply `redactors.Response.Body` to `view.Request.Body` in `callPayload`
- `internal/curl/render.go` (modify) — `bodyArgs` emits `--data @file` / `--data @-` / `--data-raw <inline>`
...
1. `--body '{"refresh_token":"CANARY"}'` written to a file, called with `--body @file`: stdout
   ...
   `stdout: "body":"{\"refresh_token\":\"CANARY\"}"` vs `history: "data":"{\"refresh_token\":\"<redacted>\"}"`.
...
2. Thread the redactor into `callPayload` (`call.go:453`) and apply `redactors.Response.Body` to
   ...
   `(req, resp, result)` today and never receives a redactor; `redactors` is built at `call.go:164`
   and used at 177 (history) and 182 (the *response* view), which is exactly the asymmetry.
3. `bodyArgs` (`render.go:123`) switches on origin. Update the comment at `render.go:118-122`,
   which explains why `--data-raw` is always used; that rationale now applies only to the argv case.
4. **Also fix the two false comments this exposes**, or Task 23 will find them:
...
**Implementation files:**
- `internal/corpus/entry.go` (modify) — `NewEntry` (line 151) takes `Observed{Status, Headers, Body, TimingMS}` instead of `*curl.Response`; drop the `internal/curl` import at line 24
- `cmd/talaria/call.go`, `cmd/talaria/history.go` (modify) — fill `Observed` from `curl.Response`

**Red — write failing tests:**
1. A test asserting `go list -deps ./internal/corpus` does not contain `internal/curl`. **It passes
   today only because nothing checks it** — `boundary_test.go`'s `shared` list is
   `{internal/operation, internal/validate}` and `corpus` is only in the `forbidden` list. Write
   the failing guard first.
...
**Implementation files:**
- `internal/corpus/store.go` (modify) — bound the read in `Read` (line 181), `storedIDs` (160) and `trim` (247)
...
1. A `history.jsonl` whose single line carries a 200 MB `data` field: `history`, `history show` and
   ...
   never the process."*
2. A base64 `data` that decodes to more than `MaxBody`: `Body.Bytes` returns an error rather than
   allocating. `MaxBody` is enforced only at **write** today (`entry.go:258-261`).
3. A file larger than the whole-store bound is refused with a clear error, not an OOM.
4. Entries under the bound are unaffected — no regression in the 19 existing store tests.

**Adversarial — what does hostile or malformed input do here?**
The history file is untrusted: it may have been written by another project, copied from another
machine, or hand-edited. It is also user-writable by design (`entry.go:115-116`).
...
3. An oversized fetch writes **nothing** to the cache. `writeCache` has no size or count bound, and
   `TestLoaderDoesNotCacheFailedFetches` already asserts the failure case — extend it.
...
3. **Redirects.** `client.Get` follows up to 10 by default, cross-scheme and cross-host, and the
   result is cached under the *original* URL's hash. Assert a redirect chain is bounded and that a
   redirect to a non-http(s) scheme is refused. `grep CheckRedirect` returns zero hits today.
4. A server that sends one byte per second for 30 seconds — `fetchTimeout` bounds the clock but not
   the volume; confirm both bounds exist independently.
...
3. Set `CheckRedirect` on the default client to bound the chain and refuse non-http(s) hops.
...
**Implementation files:**
- `internal/curl/config.go` (modify) — `document.b` becomes a `[]byte` the document owns; `BuildConfigWith` lines 120-121; `discard` (370); `cleanupWith` (382); doc comment lines 85-88
...
2. `discard()` (the failure path at `config.go:116`) also zeroes — a build that fails part-way
...
designates as the sole holder of resolved secrets. State it and move on.
...
### Task 12: Compaction — tasks 7–11

**Depends on:** Task 11

**Deliverable is negative.** No new behaviour, no new files, no new tests beyond ones that replace
several others. Green before and after; net-negative diff.

1. Read `.ralph/refactor-backlog.md` and drain it. Delete each entry as you resolve it; leave what
   you deliberately skipped with one line on why.
2. Read every file touched since Task 6.
3. Consolidate duplication. Specific suspects: bounded-read helpers now exist in both
   `internal/corpus` and `internal/spec` and should share one shape; `Observed` construction from a
   `curl.Response` may have been written at two `cmd/talaria` call sites; body-origin switching may
   appear in both `render.go` and `config.go`.
4. Move logic out of `cmd/talaria` into the package that owns it. Measure the line count and put it
   in the commit message.
5. Delete commented-out code, superseded helpers, and comments asserting unenforced properties.
6. Record the patterns in `CLAUDE.md`.

**Verify:** `go test ./...` green, net line delta in the commit message.
...
1. **Finding 19 — the exit-4 stage.** `TestErrorPathsDoNotLeakTheCredential`'s `stages` table
   ...
   validation **is** built, at `cmd/talaria/call.go:215`, exit 4. The `run --report junit` half of
   this finding is gone with `run`; do not reintroduce it.
...
must not.
1. The validation error path quotes the request — §5a names *"validation errors quoting the request"*
   as a leak channel and adds *"Test explicitly — error paths are where redaction bugs live."*
   Assert the canary is absent from the error message, not merely from the happy-path envelope.
...
`history.go:290-299` and `history.go:330-345` (recorded URLs, headers and **raw bodies** — `bodyLine`
returns `body.Data` verbatim), `uses.go:56`, `describe.go:192` (spec descriptions routinely contain
real newlines), `call.go:469-486`, `auth.go:102`.
...
**Test files:**
- `cmd/talaria/main_test.go` (modify) — signal disposition restored after the first signal
- `internal/request/body_test.go` (modify) — stdin read honours the context

**Implementation files:**
- `cmd/talaria/root.go` (modify) — `run` at line 201, `signal.NotifyContext` at 206
...
**Green — minimal implementation:**
1. In `run`, after `signal.NotifyContext`, add `go func() { <-ctx.Done(); stop() }()` so the first
   signal restores default disposition and a second one terminates.
...
5. Assert the bounded wait **does not** drop history silently: `recordCall` already downgrades an
   `Append` failure to a stderr warning, so a bounded wait costs a warning line, not a silent loss —
   assert the warning appears.

**Green — minimal implementation:**
1. `lock(ctx, path)` loops on `LOCK_EX|LOCK_NB` with a short sleep against a bounded deadline and
   the caller's context.
2. Thread the context from `Append` through; update the comment at `lock_unix.go:33-35`, which
...
### Task 18: Compaction — tasks 13–17

**Depends on:** Task 17

**Deliverable is negative.** Green before and after; net-negative diff.

1. Read and drain `.ralph/refactor-backlog.md`; delete resolved entries, leave skipped ones with a
   reason. Tasks 15 and 16 were explicitly told to record deferred size bounds there — resolve or
   restate them.
2. Read every file touched since Task 12.
3. Consolidate duplication. Suspects: context-threading boilerplate added in tasks 15, 16 and 17;
   bounded-wait / bounded-read helpers; cell sanitisation if it was written per-renderer rather
   than once.
4. Move logic out of `cmd/talaria`. Measure and report.
5. Delete commented-out code and unenforced-invariant comments. **Note:** Task 23 is the systematic
   sweep for these — do not pre-empt it, just remove what you touched.
6. Record the patterns in `CLAUDE.md`.
...
### Task 19: History never lies about what it recorded — findings 23 and 24
...
5. Two entries never share an id: assert `history replay <id>` and `history show <id>` resolve to
   the same entry. `selectEntry` (`cmd/talaria/history.go:419`) resolves an id to the *newest*
   match, so a duplicate makes replay silently re-issue a different request than show displayed.

**Adversarial — what does hostile or malformed input do here?**
The store is a file on a filesystem that can be full, read-only, or failing.
...
5. A file whose every line is valid but whose ids all collide already — `uniqueID` must terminate,
   not loop.
6. Assert the **new** error taxonomy is honest in both directions: nil must mean "on disk", and
...
**Why:** Both findings make the tool lie about history. 23 reports "not recorded" for a call that
was, so an operator re-runs a mutation; 24 lets `history replay <id>` re-issue a different request
than `history show <id>` displayed — precisely the failure the id field exists to prevent.
...
7. Concurrent `talaria` processes revalidating the same URL — `writeCache` is already temp+rename;
   keep that property and assert it.
...
4. Thread `--refresh` from `root.go` through `loadSpec` into an explicit `spec.Loader` — the
   package-level `spec.Load` discards the `Loader`, so this is the one structural change needed.
...
### Task 21: `history` distinguishes a 0 ms call from no response — finding 31
...
**Red — write failing tests:**
1. An entry whose response took 0 ms renders `"timing_ms": 0` in `history --output json`. Today
   `int64` with `omitempty` drops the field, so an agent cannot tell "0 ms" from "no response
   observed".
2. An entry with **no** response omits the field entirely.
3. The two cases are distinguishable in one parse — assert both in the same test.

**Adversarial — what does hostile or malformed input do here?**
The value is read back from the untrusted history file.
1. A stored `timing_ms` that is negative, or `int64` max.
2. A stored `timing_ms` that is a JSON string or null rather than a number — the entry is skipped
   as malformed rather than rendered as garbage.
3. An entry with a `response` object present but empty.

**Green — minimal implementation:**
1. Make `historyEntryView.TimingMS` a `*int64`, set only when `entry.Response != nil`, matching what
...
**Why:** A call against a local twin or `127.0.0.1` rounds to 0 ms and loses the field, so an agent
reading history cannot distinguish a fast call from an unobserved one.

---

### Task 22: One pass over the history file per append — finding 16
...
**Implementation files:**
- `internal/corpus/store.go` (modify) — `Append` at 99, `uniqueID` at 138, `storedIDs` at 160, `trim` at 247
...
### Task 23: No comment asserts an invariant no test enforces
...
1. `internal/curl/sweep.go:55-56` — *"Info is lstat here, so a symlink somebody planted under one
   of these names is judged and removed as the link it is, never followed."* `TestSweepStaleRemovesOnlyTalariaTempFilesPastTheGrace` (`exec_test.go:593-633`) plants only
...
4. `internal/curl/config.go:85-87` — the buffer-zeroing claim. **Task 11 fixes this**; confirm it
...
11b. `internal/curl/render.go:4-7` — the package doc claims an emitted command "never calls
...
12. `internal/corpus/lock_unix.go:21-22` — *"exactly as separate talaria processes do."* The
    goroutine half is covered; the multi-process half is asserted by argument only.
13. `internal/operation/index.go:63-64` — *"whatever order the two operations appear in."*
    `index_test.go:84-91` covers one direction only.
14. `internal/curl/version.go:65-67` — *"these cannot fail on anything but an implausibly long
    number."* Silent `return nil` on `Atoi` failure, no test.
15. `internal/clierr/clierr.go:128-130` — *"marshalling cannot fail."* Structurally true, benign
...
2. Items 4-7 are verification-only: assert the earlier tasks actually landed.
3. For items 8-15, either write the test or rewrite the comment to claim only what is true. **Record
   the decision for each in the commit message** — a silent drop is indistinguishable from an
   oversight.
...
3. Add the rule to `CLAUDE.md` if it is not already stated strongly enough — it is, so instead
   record *where* the sweep looked, so the next one does not start over.
...
### Task 24: The credential firewall holds end to end
...
4. A spec fetched over HTTP is bounded (Task 10), cached, revalidated and refreshable (Task 20),
   and leaves no credential in the cache (`canary_test.go:929`).
...
**Verify:** `go test ./...` and `go build ./...` and `test -z "$(gofmt -l .)" && go vet ./...`
...
### Task 25: Compaction — end of phase

**Depends on:** Task 24

**Deliverable is negative.** Green before and after; net-negative diff. This is the last task in the
phase, so it also leaves the tree in the state phase 2b starts from.

1. Read and drain `.ralph/refactor-backlog.md` completely. Anything left must carry an explicit
   one-line reason and be visible to phase 2b.
2. Read every file touched since Task 18.
3. Consolidate across the whole phase, not just the last five tasks. By now the same shapes exist in
   several packages: bounded reads (`corpus`, `spec`), bounded waits (`corpus`, `curl`),
   context threading (`request`, `curl`, `corpus`), and host/URL normalisation.
4. **Measure `cmd/talaria`.** It held 1,589 of 5,511 non-comment production lines (28%) and 12 view
   structs at the start of this phase, and CLAUDE.md forbids growing it. Tasks 3, 4, 7 and 20 all
   touched it. Report the before/after count in the commit message; if it grew, move something down.
5. Delete commented-out code and superseded helpers. Task 23 already swept unenforced comments —
...
6. Update `CLAUDE.md` with every pattern this phase established, and update the scope note if the
   ...
   and the fact that `replayableEnv` is gone (which resolves finding 18 without a design amendment).
7. Confirm `.ralph/stack.json`'s three command fields still match `.github/workflows/ci.yml`
   byte-for-byte — `internal/ci/workflow_test.go` fails if they drift, and Task 20 added a flag but
   should not have changed a command.
